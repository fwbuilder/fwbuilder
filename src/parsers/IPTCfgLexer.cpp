...
void IPTCfgLexer::mNUMBER(bool _createToken) {
	...
	if ( synPredMatched189 ) {
		...
		mNUM_3DIGIT(false);
		if ( inputState->guessing==0 ) {
#line 1229 "iptables.g"
			_ttype = IPV4;
#line 1027 "IPTCfgLexer.cpp"
		}
		}
	}
...
void IPTCfgLexer::mRSOURCE(bool _createToken) {
	...
	match("--rsource");
	if ( inputState->guessing==0 ) {
#line 1258 "iptables.g"
		_ttype = UNSUPPORTED_OPTION;
#line 1390 "IPTCfgLexer.cpp"
	}
	if ( _createToken && _token==ANTLR_USE_NAMESPACE(antlr)nullToken && _ttype!=ANTLR_USE_NAMESPACE(antlr)Token::SKIP ) {
	   _token = makeToken(_ttype);
	   _token->setText(text.substr(_begin, text.length()-_begin));
