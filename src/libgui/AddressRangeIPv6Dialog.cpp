...
AddressRangeIPv6Dialog::AddressRangeIPv6Dialog(QWidget *parent):
        BaseObjectDialog(parent)
{
    m_dialog = new Ui::AddressRangeIPv6Dialog_q;
...
AddressRangeIPv6Dialog::~AddressRangeIPv6Dialog()
{
    delete m_dialog;
}

void AddressRangeIPv6Dialog::loadFWObject(FWObject *o)
{
    obj=o;
    AddressRangeIPv6 *s = dynamic_cast<AddressRangeIPv6*>(obj);
    assert(s!=NULL);
...
void AddressRangeIPv6Dialog::validate(bool *res)
{
    *res=true;
    ...
    try
    {
        InetAddr(AF_INET6,m_dialog->rangeStart->text().toLatin1().constData());
        InetAddr(AF_INET6,m_dialog->rangeEnd->text().toLatin1().constData());
...
void AddressRangeIPv6Dialog::applyChanges()
{
    std::unique_ptr<FWCmdChange> cmd(new FWCmdChange(m_project, obj));
    FWObject* new_state = cmd->getNewState();

    AddressRangeIPv6 *s = dynamic_cast<AddressRangeIPv6*>(new_state);
    assert(s!=NULL);
